...
# The script also runs a TRT latency evaluation.
...
# Preserve existing relative-path assumptions for config/weights/onnx output.
...
nrx_inputs, bits, u, h = generator(batch_size, 0.0)
...
batch_size = 1
num_prbs = [num_prbs, num_prbs, num_prbs]
num_dmrs_time = [
    len(dmrs_ofdm_pos[-1]),
    len(dmrs_ofdm_pos[-1]),
    len(dmrs_ofdm_pos[-1]),
